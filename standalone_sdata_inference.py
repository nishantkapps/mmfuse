...
def main():
    ser = serial.Serial(
    port="COM3",      # change this to your port (COM4, COM5, etc.)
    baudrate=115200,
    timeout=1
)
    ...
    state_dict = ckpt  # flat state_dict

    # ---- Infer dimensions ----
    vision_dim = 3584
    audio_dim = 768
    text_dim = state_dict["fusion.projections.text.0.weight"].shape[1]

    fusion_dim = state_dict["fusion.projections.audio.0.weight"].shape[0]
    num_classes = state_dict["action_head.fc.weight"].shape[0]
    ...
    emg_enc = EMGSensorEncoder(output_dim=256, input_features=4).to(device)
    ...
    # ---- Load weights ----
    pressure_enc.load_state_dict({
        k.replace("pressure.", ""): v
        for k, v in state_dict.items()
        if k.startswith("pressure.")
    })

    emg_enc.load_state_dict({
        k.replace("emg.", ""): v
        for k, v in state_dict.items()
        if k.startswith("emg.")
    })
    ...
    movement_dict = {
        k.replace("movement_head.", ""): v
        for k, v in state_dict.items()
        if k.startswith("movement_head.")
    }

    has_movement = len(movement_dict) > 0
    if has_movement:
        movement_head.load_state_dict(movement_dict)
    ...
    for i, sample_path in enumerate(samples[:n]):
        ...
        with torch.no_grad():
            ...
            if has_movement:
                ...
                serial_msg = f"{delta_along:.4f},{delta_lateral:.4f},{magnitude:.4f}\n"
                ser.write(serial_msg.encode("utf-8"))
                print(f"  Sent to serial: {serial_msg.strip()}")
